...
from custom_components.homekit.pyhap.const import CATEGORY_SPEAKER
from custom_components.homekit.type_lights import (
...
class HatchRestPlus(HomeAccessory):
    ...
    def __init__(self, *args: Any) -> None:
        ...
        if self.linked_light:
            _LOGGER.debug(
                "%s: Found linked light %s", self.entity_id, self.linked_light
            )
            ...
            if light_state:
                ...
                serv_light.configure_char(
                    CHAR_NAME,
                    value=f"{prefix} Nightlight"[:MAX_NAME_LENGTH],
                )
        ...
        if self.linked_media_player:
            ...
            if media_player_state:
                ...
                serv_audio.configure_char(
                    CHAR_CONFIGURED_NAME,
                    value=f"{prefix} Sound Machine"[:MAX_NAME_LENGTH],
                )
                serv_audio.configure_char(
                    CHAR_SLEEP_DISCOVER_MODE,
                    value=True,
                ...
                self.audio_char_active = serv_audio.configure_char(
                    CHAR_ACTIVE,
                    value=False,
                    setter_callback=self._set_audio_state,
                )

                self.audio_char_input_source = serv_audio.configure_char(
                    CHAR_ACTIVE_IDENTIFIER,
                    setter_callback=self._set_audio_input_source,
                )
                self.sources = []
                source_list = media_player_state.attributes.get(
                    ATTR_SOUND_MODE_LIST, []
                )
                if source_list:
                ...
                for index, source in enumerate(self.sources):
                    ...
                    serv_input.configure_char(
                        CHAR_INPUT_DEVICE_TYPE,
                        value=INPUT_DEVICE_TYPES[source[CONF_INPUT_DEVICE_TYPE]],
                    )

                    serv_input.configure_char(
                        CHAR_INPUT_SOURCE_TYPE,
                        value=INPUT_SOURCE_TYPES[source[CONF_INPUT_SOURCE_TYPE]],
                    )
                    ...
                    serv_input.configure_char(
                        CHAR_NAME,
                        value=source[CONF_SOURCE][:MAX_NAME_LENGTH],
                    )

                    _LOGGER.debug(
                        "%s: Added source %s", self.entity_id, source[CONF_SOURCE]
                    )
                ...
                serv_speaker.configure_char(
                    CHAR_NAME,
                    value=f"{prefix} Speaker"[:MAX_NAME_LENGTH],
                )
                serv_speaker.configure_char(
                    CHAR_ACTIVE,
                    value=1,
                ...
                self.speaker_char_mute = serv_speaker.configure_char(
                    CHAR_MUTE,
                    value=False,
                    setter_callback=self._set_speaker_mute,
                )
                serv_speaker.configure_char(
                    CHAR_VOLUME_CONTROL_TYPE,
                    value=1,
                ...
                self.speaker_char_volume_selector = serv_speaker.configure_char(
                    CHAR_VOLUME_SELECTOR,
                    setter_callback=self._set_speaker_volume_step,
                )
                self.speaker_char_volume = serv_speaker.configure_char(
                    CHAR_VOLUME,
                    setter_callback=self._set_speaker_volume,
                )
    ...
    def _async_update_light_state(self, new_state):
        ...
        # Handle brightness
        if (brightness := attributes.get(ATTR_BRIGHTNESS)) is not None and isinstance(
    ...
    def _async_update_media_player_state(self, new_state):
        ...
        _LOGGER.debug(
            "%s: Set current state to %s", self.linked_media_player, current_state
        )
        ...
        if self.sources:
            ...
            _LOGGER.debug(
                "%s: Set current input to %s", self.linked_media_player, source_name
            )
        ...
        if isinstance(volume_level, (float, int)):
            ...
            _LOGGER.debug(
                "%s: Set current volume level to %s", self.linked_media_player, volume
            )
    ...
    def _async_send_light_events(self, *_):
        ...
        if service == SERVICE_TURN_OFF:
            self.async_call_service(
                LIGHT_DOMAIN,
                service,
                {ATTR_ENTITY_ID: self.linked_light},
                ", ".join(events),
            )
            return
    ...
    def _set_audio_input_source(self, value):
        ...
        self.async_call_service(MEDIA_PLAYER_DOMAIN, SERVICE_SELECT_SOUND_MODE, params)

    def _set_speaker_mute(self, value):
        """Move switch state to value if call came from HomeKit."""
        _LOGGER.debug(
            '%s: Set switch state for "toggle_mute" to %s',
            self.linked_media_player,
            value,
        )
        params = {
            ATTR_ENTITY_ID: self.linked_media_player,
