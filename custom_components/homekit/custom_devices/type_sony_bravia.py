...
from custom_components.homekit.pyhap.const import CATEGORY_TELEVISION
...
MEDIA_PLAYER_KEYS = {
    0: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Rewind"}],
    1: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Forward"}],
    2: [MEDIA_PLAYER_DOMAIN, SERVICE_MEDIA_NEXT_TRACK, {}],
    3: [MEDIA_PLAYER_DOMAIN, SERVICE_MEDIA_PREVIOUS_TRACK, {}],
    4: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Up"}],
    5: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Down"}],
    6: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Left"}],
    7: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Right"}],
    8: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Confirm"}],
    9: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Return"}],
    10: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Exit"}],
    11: [MEDIA_PLAYER_DOMAIN, SERVICE_MEDIA_PLAY_PAUSE, {}],
    15: [SONY_BRAVIA_DOMAIN, SERVICE_SEND_COMMAND, {ATTR_COMMAND: "Display"}],
}
...
class SonyBRAVIA(HomeAccessory):
    ...
    def __init__(self, *args) -> None:
        ...
        self.char_mute = serv_speaker.configure_char(
            CHAR_MUTE,
            value=False,
            setter_callback=self.set_mute,
        )

        serv_speaker.configure_char(
            CHAR_NAME,
            value=f"{self.display_name} Volume",
        )
    ...
    def async_update_state(self, new_state):
        ...
        if CHAR_VOLUME in self.speaker_chars:
            ...
            if isinstance(volume_level, (float, int)):
                ...
                _LOGGER.debug(
                    "%s: Set current volume level to %s", self.entity_id, volume
                )
