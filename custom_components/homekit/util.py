...
BASIC_INFO_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(CONF_ACCESSORY_INFO_MANUFACTURER): cv.string,
        vol.Optional(CONF_ACCESSORY_INFO_MODEL): cv.string,
        vol.Optional(CONF_ACCESSORY_INFO_SERIAL_NUMBER): cv.string,
...
DEVICE_SCHEMA = BASIC_INFO_SCHEMA.extend(
    {
        vol.Optional(CONF_SERVICE_NAME_PREFIX): cv.string,
    }
)
...
AEOTEC_LEAK_SENSOR_SCHEMA = DEVICE_SCHEMA.extend(
    {vol.Optional(CONF_LINKED_TAMPER_SENSOR): cv.entity_domain(binary_sensor.DOMAIN)}
)

BROADLINK_REMOTE_SCHEMA = DEVICE_SCHEMA.extend(
    {vol.Optional(CONF_LINKED_HUMIDITY_SENSOR): cv.entity_domain(sensor.DOMAIN)}
)

HATCH_REST_PLUS_SCHEMA = DEVICE_SCHEMA.extend(
    {
        vol.Required(CONF_LINKED_LIGHT): cv.entity_domain(light.DOMAIN),
        vol.Required(CONF_LINKED_MEDIA_PLAYER): cv.entity_domain(media_player.DOMAIN),
    }
)
...
SMARTTHINGS_LEAK_SENSOR_SCHEMA = DEVICE_SCHEMA.extend(
    {vol.Optional(CONF_LINKED_TEMPERATURE_SENSOR): cv.entity_domain(sensor.DOMAIN)}
)
...
TOTAL_CONNECT_SCHEMA = DEVICE_SCHEMA.extend(
    {vol.Optional(CONF_LINKED_TAMPER_SENSOR): cv.entity_domain(binary_sensor.DOMAIN)}
)
...
TUYA_STAR_PROJECTOR_SCHEMA = DEVICE_SCHEMA.extend(
    {
        vol.Optional(CONF_LINKED_LIGHT_COLOR): cv.entity_domain(light.DOMAIN),
...
def validate_entity_config(values: dict) -> dict[str, dict]:
    ...
    for entity_id, config in values.items():
        ...
        if device:
            if device == DEVICE_AEOTEC_LEAK_SENSOR:
                config = AEOTEC_LEAK_SENSOR_SCHEMA(config)

            elif device == DEVICE_BROADLINK_REMOTE:
                config = BROADLINK_REMOTE_SCHEMA(config)
            ...
            elif device == DEVICE_SMARTTHINGS_LEAK_SENSOR:
                config = SMARTTHINGS_LEAK_SENSOR_SCHEMA(config)

            elif device == DEVICE_SONY_BRAVIA:
                config = SONY_BRAVIA_SCHEMA(config)
                ...
                config = TUYA_STAR_PROJECTOR_SCHEMA(config)

            else:
                config = DEVICE_SCHEMA(config)

        elif domain in ("alarm_control_panel", "lock"):
