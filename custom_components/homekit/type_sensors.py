...
class BinarySensor(HomeAccessory):
    ...
    def __init__(self, *args: Any) -> None:
        ...
        chars = [service_char[1]]

        self.support_active = attrs.get(ATTR_AVAILABLE) in (True, False)
        if self.support_active:
            chars.append(CHAR_STATUS_ACTIVE)

        self.support_low_battery = attrs.get(ATTR_LOW_BATTERY) in (True, False)
        if self.support_low_battery:
            chars.append(CHAR_STATUS_LOW_BATTERY)

        self.support_tamper_detected = attrs.get(ATTR_TAMPER_DETECTED) in (True, False)
        self.support_tampered = attrs.get(ATTR_TAMPERED) in (True, False)
        if self.support_tamper_detected or self.support_tampered:
            chars.append(CHAR_STATUS_TAMPERED)

        self.format = service_char.format
        service = self.add_preload_service(service_char.service, chars)
        ...
        if self.support_active:
            self.char_active = service.configure_char(CHAR_STATUS_ACTIVE, value=0)
        if self.support_low_battery:
            self.char_low_battery = service.configure_char(
                CHAR_STATUS_LOW_BATTERY, value=0
            )
        if self.support_tamper_detected or self.support_tampered:
            self.char_tampered = service.configure_char(CHAR_STATUS_TAMPERED, value=0)
        # Set the state so it is in sync on initial
        # GET to avoid an event storm after homekit startup
        self.async_update_state(state)
    ...
    def async_update_state(self, new_state: State) -> None:
        ...
        if self.support_active:
            active = new_state.attributes.get(ATTR_AVAILABLE)
            if isinstance(active, bool):
                self.char_active.set_value(active)

        if self.support_low_battery:
            low_battery = new_state.attributes.get(ATTR_LOW_BATTERY)
            if isinstance(low_battery, bool):
                self.char_low_battery.set_value(low_battery)

        if self.support_tamper_detected:
            tamper_detected = new_state.attributes.get(ATTR_TAMPER_DETECTED)
            if isinstance(tamper_detected, bool):
                self.char_tampered.set_value(tamper_detected)

        if self.support_tampered:
            tampered = new_state.attributes.get(ATTR_TAMPERED)
            if isinstance(tampered, bool):
                self.char_tampered.set_value(tampered)
