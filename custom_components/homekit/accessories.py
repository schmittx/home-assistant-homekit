...
CUSTOM_DEVICES = {
    DEVICE_AEOTEC_LEAK_SENSOR: "AeotecLeakSensor",
    DEVICE_BROADLINK_REMOTE: "BroadlinkRemote",
    DEVICE_HATCH_REST_PLUS: "HatchRestPlus",
    DEVICE_RATGDO: "RATGDO",
    DEVICE_SMARTTHINGS_BUTTON: "SmartThingsButton",
    DEVICE_SMARTTHINGS_LEAK_SENSOR: "SmartThingsLeakSensor",
    DEVICE_SONY_BRAVIA: "SonyBRAVIA",
    DEVICE_TOTAL_CONNECT_CONTACT_SENSOR: "TotalConnectContactSensor",
    DEVICE_TOTAL_CONNECT_SECURITY_SYSTEM: "TotalConnectSecuritySystem",
    DEVICE_TOTAL_CONNECT_SMOKE_SENSOR: "TotalConnectSmokeSensor",
    DEVICE_TUYA_STAR_PROJECTOR: "TuyaStarProjector",
}
SWITCH_TYPES = {
...
def get_accessory(  # noqa: C901
    ...
    domain = config.get(CONF_DEVICE, state.domain)
    features = state.attributes.get(ATTR_SUPPORTED_FEATURES, 0)

    if domain in CUSTOM_DEVICES:
        _LOGGER.debug(
            'Found "%s" custom device type for "%s"',
            domain,
            state.entity_id,
        )
        a_type = CUSTOM_DEVICES[domain]

    elif domain == "alarm_control_panel":
        a_type = "SecuritySystem"

    elif domain in ("binary_sensor", "device_tracker", "person"):
        a_type = "BinarySensor"
    ...
    elif domain == "remote" and features & RemoteEntityFeature.ACTIVITY:
...
class HomeAccessory(Accessory):  # type: ignore[misc]
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Manufacturer
        if self.config.get(CONF_ACCESSORY_INFO_MANUFACTURER) is not None:
            manufacturer = self.config[CONF_ACCESSORY_INFO_MANUFACTURER]
        elif self.config.get(ATTR_MANUFACTURER) is not None:
        ...
        # Model
        if self.config.get(CONF_ACCESSORY_INFO_MODEL) is not None:
            model = self.config[CONF_ACCESSORY_INFO_MODEL]
        elif self.config.get(ATTR_MODEL) is not None:
        ...
        # Serial Number
        if self.config.get(CONF_ACCESSORY_INFO_SERIAL_NUMBER) is not None:
            serial_number = self.config[CONF_ACCESSORY_INFO_SERIAL_NUMBER]
        elif self.config.get(ATTR_HW_VERSION) is not None:
            serial_number = self.config[ATTR_HW_VERSION]
        ...
        serv_battery = self.add_preload_service(SERV_BATTERY_SERVICE, battery_chars)
        prefix = self.config.get(CONF_SERVICE_NAME_PREFIX, self.display_name)
        serv_battery.configure_char(CHAR_NAME, value=f"{prefix} Battery")
